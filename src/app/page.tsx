        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-12">
            <div className="space-y-2">
              <h2 className="text-3xl md:text-4xl font-black text-primary uppercase tracking-tighter">Últimas Publicaciones</h2>
              <p className="text-muted-foreground font-medium">Actualidad técnica y normativa para el sector hospitality.</p>
            </div>
            <Button variant="outline" className="border-primary text-primary font-bold gap-2" asChild>
              <Link href="/blog">Ir al Blog completo <ChevronRight className="w-4 h-4" /></Link>
            </Button>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[1, 2, 3].map((i) => (
              <div key={i} className="bg-white rounded-3xl p-6 shadow-sm border group hover:shadow-xl transition-all cursor-pointer">
                <div className="aspect-video bg-slate-100 rounded-2xl mb-6 overflow-hidden relative">
                  <Image src={`https://picsum.photos/seed/blog-${i}/600/400`} alt="Blog thumbnail" fill className="object-cover group-hover:scale-105 transition-transform duration-500" />
                </div>
                <Badge variant="outline" className="text-[10px] font-black uppercase mb-3">Normativa</Badge>
                <h3 className="text-xl font-bold mb-3 group-hover:text-accent transition-colors leading-tight">Novedades en el cumplimiento del RD 3/2023</h3>
                <p className="text-sm text-muted-foreground line-clamp-2 mb-4">Descubre cómo los nuevos criterios técnicos afectan a la gestión de piscinas...</p>
                <Link href="/blog" className="text-xs font-black text-primary uppercase tracking-widest flex items-center gap-2">Leer más <ChevronRight className="w-3 h-3" /></Link>
              </div>
            ))}
          </div>
        </div>
