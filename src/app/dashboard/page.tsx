...
import { OrderItem, LogbookEntry, LogbookStatus, Supplier, CatalogItem } from '@/lib/types';
...
export default function DashboardPage() {
  ...
  // States for unified Order Modal
  ...
  // Order Handlers (Sync with orders/page.tsx)
  ...
  const handleSelectOrderSuggestion = (index: number, suggestion: { name: string; unit: string }) => {
    const updated = [...manualItems];
    updated[index] = { 
      ...updated[index], 
  ...
  return (
    <DashboardLayout>
      ...
      {/* NEW ORDER MODAL - Unified with Orders Page */}
      <Dialog open={isOrderModalOpen} onOpenChange={setIsOrderModalOpen}>
        <DialogContent className="sm:max-w-[800px] max-h-[90vh] flex flex-col p-0 overflow-hidden" onCloseAutoFocus={(e) => e.preventDefault()}>
          <DialogHeader className="p-6 bg-slate-50 border-b">
            <DialogTitle>Nuevo Pedido Técnico</DialogTitle>
            <DialogDescription>Genera un borrador de pedido para suministros con autocompletado.</DialogDescription>
          </DialogHeader>
          ...
          <div className="flex-1 overflow-y-auto p-6 pb-32 space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleAddOrderRow} className="h-7 text-[10px] font-bold">
                ...
                {manualItems.map((item, idx) => (
                  <div key={idx} className="flex gap-3 items-end group">
                          ...
                          {combinedSuggestions
                            ...
                            .map((s, sIdx) => (
                              <button 
                                ...
                                onClick={() => handleSelectOrderSuggestion(idx, s)}
                              >
                                <div className="flex flex-col">
                                  <span className="font-medium">{s.name}</span>
                                  {s.stock !== undefined && (
                                    <span className="text-[9px] text-accent font-bold uppercase tracking-tighter">Stock actual: {s.stock} {s.unit}</span>
                                  )}
                                </div>
                    ...
                    <div className="w-28 space-y-1.5">
                      ...
                      <Select value={item.unit} onValueChange={(v) => handleUpdateOrderItem(idx, 'unit', v)}>
          ...
          <DialogFooter className="p-6 bg-slate-50 border-t">
            ...
            <Button className="bg-accent px-8 font-bold text-white shadow-lg" onClick={handleCreateOrder}>
              Registrar Pedido
            </Button>
          </DialogFooter>
        </DialogContent>
