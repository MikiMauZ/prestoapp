...
import { collection, query, orderBy, limit, doc, where, serverTimestamp, arrayUnion, addDoc, updateDoc, increment } from 'firebase/firestore';
...
export default function DashboardPage() {
  ...
  const { data: pendingIncidents, isLoading: loadingIncidents, error: incidentsError } = useCollection<LogbookEntry>(incidentsQuery);
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast({ title: "Evidencias en R2", description: "Imágenes cargadas correctamente." });
    } catch (error) {
      toast({ variant: "destructive", title: "Error R2", description: "Fallo al subir imágenes." });
    } finally {
          ...
          <form onSubmit={handleCreateIncident} className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button type="button" onClick={() => fileInputRef.current?.click()} className="w-12 h-12 rounded border-2 border-dashed flex items-center justify-center text-muted-foreground hover:text-accent">
