...
export default function KnowledgeBasePage() {
  ...
  const [articleForQR, setArticleForQR] = useState<any>(null);
  ...
  // Protección contra congelamiento de pantalla
  useEffect(() => {
    const cleanup = () => {
      document.body.style.pointerEvents = 'auto';
      document.body.style.overflow = 'auto';
    };
    if (!isDialogOpen && !isDeleteDialogOpen && !isQRModalOpen) {
      cleanup();
    }
    return cleanup;
  }, [isDialogOpen, isDeleteDialogOpen, isQRModalOpen]);
  ...
  const handleDelete = () => {
    ...
    const idToDelete = selectedArticle.id;
    setSelectedArticle(null);
    setIsDeleteDialogOpen(false);

    setTimeout(() => {
      const artRef = doc(db, 'tenants', profile.tenantId!, 'knowledgeBase', idToDelete);
  ...
  const handleShowQR = (e: React.MouseEvent, article: any) => {
    e.stopPropagation();
    setArticleForQR(article);
    setIsQRModalOpen(true);
  };

  const publicUrl = articleForQR && profile?.tenantId 
    ? `${window.location.origin}/kb/${profile.tenantId}/${articleForQR.id}`
    : '';
    ...
    <DashboardLayout>
      <div className="space-y-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-muted-foreground font-medium">Guías técnicas y manuales operativos por código QR.</p>
          </div>
          ...
          <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-2">
                <Button variant="outline" size="sm" onClick={() => setIsQRModalOpen(true)} className="gap-2 text-primary">
                  <QrCode className="w-4 h-4" /> QR Ficha
                </Button>
                ...
                <Card key={article.id} className="border-none shadow-md hover:shadow-xl transition-all group flex flex-col cursor-pointer overflow-hidden" onClick={() => setSelectedArticle(article)}>
                  ...
                  <CardHeader className="pb-2">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-primary" onClick={(e) => handleShowQR(e, article)}>
      ...
      <Dialog open={isQRModalOpen} onOpenChange={setIsQRModalOpen}>
        <DialogContent className="sm:max-w-[400px]" onCloseAutoFocus={(e) => e.preventDefault()}>
          <DialogHeader>
            <DialogTitle>Código QR Técnico</DialogTitle>
            <DialogDescription>Escanea para acceder a esta guía desde cualquier dispositivo en el punto de avería.</DialogDescription>
          </DialogHeader>
          <div className="flex flex-col items-center justify-center p-8 space-y-6 bg-slate-50 rounded-2xl border-2 border-dashed">
            ...
            <div className="text-center">
              <p className="font-black text-primary uppercase text-lg">{articleForQR?.title || selectedArticle?.title}</p>
              <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-1">Acceso Directo al Manual</p>
            </div>
          </div>
          <DialogFooter>
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent onCloseAutoFocus={(e) => e.preventDefault()}>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta acción es irreversible. Se borrará permanentemente la guía del sistema.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
