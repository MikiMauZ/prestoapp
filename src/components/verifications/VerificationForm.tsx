...
export default function VerificationForm() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!files || files.length === 0 || !activeParamForPhoto) return;
    ...
    try {
      ...
      setPhotos(prev => ({ 
        ...prev, 
        [paramName]: [...(prev[paramName] || []), ...urls] 
      }));
      toast({
        title: "Evidencias guardadas",
        description: `${urls.length} foto(s) del patrón cargada(s) correctamente.`,
      });
    } catch (error) {
      ...
      toast({
        ...
        description: "No se pudieron subir las evidencias. Por favor, revisa tu conexión.",
      });
    } finally {
  ...
  const removePhoto = (paramName: string, index: number) => {
    setPhotos(prev => ({
      ...prev,
  ...
  const handleSubmit = async () => {
    ...
    // Validation
    const missingPhotos = params.filter(p => !photos[p.name] || photos[p.name].length === 0);
    ...
    if (missingPhotos.length > 0 || missingRefs.length > 0 || missingReadings.length > 0) {
      toast({
        ...
        description: "Todos los patrones requieren referencia de lote, lectura y al menos una evidencia fotográfica.",
      });
      return;
    }
            ...
            <div className="space-y-6 animate-in fade-in duration-500">
              ...
              <div className="space-y-8">
                {params.map((p) => {
                  ...
                  const currentPhotos = photos[p.name] || [];
                    ...
                    <div key={p.name} className="flex flex-col gap-6 p-6 rounded-2xl border bg-secondary/20 transition-all hover:shadow-md">
                      <div className="flex flex-col lg:flex-row gap-8">
                        ...
                        <div className="w-full lg:w-80">
                          <Label className="text-[10px] font-black uppercase text-muted-foreground mb-2 block">Evidencias (Una o varias)</Label>
                          <div className="grid grid-cols-2 gap-2 mb-2">
                            {currentPhotos.map((url, idx) => (
                              <div key={idx} className="relative aspect-square rounded-lg overflow-hidden border bg-white group">
                                <img src={url} alt="Evidencia" className="absolute inset-0 w-full h-full object-cover" />
                                <button 
                                  onClick={() => removePhoto(p.name, idx)}
                            ...
                            <div 
                              ...
                              className={cn(
                                "relative aspect-square cursor-pointer border-2 border-dashed rounded-lg flex flex-col items-center justify-center gap-1 transition-all bg-white",
                                isUploading ? "opacity-50" : "border-muted-foreground/30 hover:border-accent hover:bg-slate-50"
                              )}
                            >
                              {isUploading ? (
                          ...
                          {currentPhotos.length === 0 && (
                            <p className="text-[9px] text-orange-600 font-bold italic">* Evidencia obligatoria para sellado legal</p>
                          )}
                        </div>
                      </div>
                    </div>
